...
Это демонстрационный пример реализации функции арифметических расчетов для коммуникационного сервера Эликонт-КС.
Приложение читает данные, производит расчет и отправляет результат в канал User channel Эликонт-КС. Далее эти данные могут быть переданы в смежную систему по одному из коммуникационных протоколов.
...
* cs_calc.py - стартовый файл для запуска обмена данными (на языке Python);
* аrithmetics.py - файл, в котором реализован класс для работы с лицензией Эликонт-КС (на языке Python);
...
5. В Конфигураторе Эликонт-КС открыть проект calc.elconf и загрузить его в КС.
...
CYCLE_PERIOD = 1000 | период цикла проведения расчетов (мс)
CONNECT_PERIOD = 1000 | период подключения к КС (сек)
...
## Примечание
В предложенным примере не реализована обработка качества сигналов. Все расчетные сигналы будут иметь качество Good независимо от качества исходных сигналов.
